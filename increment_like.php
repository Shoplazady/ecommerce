...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT * FROM like_comment WHERE comment_id = :comment_id AND user_id = :user_id");
        $stmt->execute(['comment_id' => $commentId, 'user_id' => $userId]);
        if ($stmt->rowCount() > 0) {
            // User has already liked the comment, delete the like
            $stmt = $conn->prepare("DELETE FROM like_comment WHERE comment_id = :comment_id AND user_id = :user_id");
            $stmt->execute(['comment_id' => $commentId, 'user_id' => $userId]);
            ...
            $stmt = $conn->prepare("INSERT INTO like_comment (comment_id, user_id) VALUES (:comment_id, :user_id)");
            $stmt->execute(['comment_id' => $commentId, 'user_id' => $userId]);
        }
